...
def _generate_unique_slug(session: Session, company_id: UUID, base_slug: str) -> str:
    """Generate a slug unique to the given company."""
    statement = select(Job.slug).where(
        Job.company_id == company_id, Job.slug.startswith(base_slug)
    )
    existing_slugs = set(session.execute(statement).scalars())
